...
class ClusteringModel:
    ...
    def find_kmeans_n_clusters(self, max_clusters: int = 20):
        """
        ...
        Returns:
            optimal_clusters(int): 최적의 클러스터 수
    ...
    def kmeans_clustering(self, n_clusters: int, train_data: pd.DataFrame, test_data: pd.DataFrame, feature_columns: List[str], label_column: str = "region"):
        """
    ...
    def find_dbscan_n_clusters(self, min_samples : int = 5):
        """
        ...
        Returns:
            best_eps: 최적의 eps 값. 클러스터가 2개 이상인 경우에만 반환
        """
        best_eps = None
        best_score = -1
    ...
    ### DBSCAN 클러스터링 수행 메서드 ###
    def dbscan_clustering(self, eps: float, train_data: pd.DataFrame, test_data: pd.DataFrame, feature_columns: List[str], label_column: str = "region", min_samples: int = 5):
        """
